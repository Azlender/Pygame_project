...
class snake():
    def __init__(self, speed, size):
        ...
        self.image.fill((random.randint(20, 255), random.randint(20, 255), random.randint(20, 255)))
        self.speed = speed
        self.size = size
        self.images = []
        self.old_pos = [[20, 20]]
    ...
    def check_apple(self, x):
        c = collide2(self.pos[0], self.pos[1], self.pos[0] + 10, self.pos[1] + 10, x[0], x[1], x[0] + 10, x[1] + 10,
                     self.size)
        return c

    def check_collisions2(self, x):
        c = collide3(self.pos[0], self.pos[1], self.pos[0] + 10, self.pos[1] + 10, x[0], x[1], x[0] + 10, x[1] + 10,
                     self.size)
        return c
    ...
    def add_apple(self):
        ...
        block.fill((random.randint(20, 255), random.randint(20, 255), random.randint(20, 255)))
...
class game():
    def __init__(self, speed, size=1):
        ...
        pg.display.set_caption('RAINDBOW SNAKE')
    ...
    def over(self):
        while 1:
            ...
            self.screen.blit(txts, txtrect)
            pg.display.update()
            self.make_button((153, 300, 100, 50), 'Заново', [(255, 255, 255), (150, 150, 150)],
...
class startmenu():
    def __init__(self):
        self.screen = pg.display.set_mode((800, 450))
        self.b1 = '(150, 300,100,50),"Играть", [(255,255,255), (255,150,255)], action = self.start'
        self.b2 = '(550, 300,100,50),"Выход", [(255,255,255), (255,255,255)], action = self.exit'
    ...
    def make_button(self, pos, text, color, action=None, textsize=20):
        ...
        circle = pg.Surface([min(rect.size) * 3] * 2, pg.SRCALPHA)
        pg.draw.ellipse(circle, (255, 255, 255), circle.get_rect(), 0)
        circle = pg.transform.smoothscale(circle, [int(min(rect.size) * 0.5)] * 2)
    ...
    def mainloop(self):
        while 1:
            ...
            self.make_text(400, 150, 'RAINDBOW SNAKE', color=(255, 255, 255), size=80, a=True)
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    sys.exit()
            for x in self.blocks:
    ...
    def start(self):
        self.b1 = '(150, 300,100,50),"Мелко", [(0,255,0), (0,150,0)], action = self.start3'
        self.b2 = '(550, 300,100,50),"Крупно", [(0,255,0), (0,150,0)], action = self.start4'
        self.buttons = [self.b1, self.b2]

    def start3(self):
        self.b1 = '(150, 300,100,50),"Легко", [(0,255,0), (0,150,0)], action = self.e'
        self.b2 = '(283, 300,100,50),"Средне", [(0,255,0), (0,150,0)], self.n'
        self.b3 = '(417, 300,100,50),"Сложно", [(0,255,0), (0,150,0)], action = self.h'
        self.b4 = '(550, 300,100,50),"Нереально", [(0,255,0), (0,150,0)], action = self.ex'
